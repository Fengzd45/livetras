...
async def process_audio_and_translate(audio_b64: str, target_langs: Dict[str, str],
    ...
    try:
        ...
        # 注意：requests 不支持异步，但此处用同步，在线程池执行或直接阻塞（影响不大）
        # 实际可改为 httpx.AsyncClient 实现异步，为简化保持同步
        files = {
            "file": ("audio.wav", wav_data, "audio/wav"),
            "model": (None, ASR_MODEL)
        }
        asr_resp = requests.post(ASR_URL, headers={"Authorization": f"Bearer {SILICONFLOW_API_KEY}"}, files=files)
